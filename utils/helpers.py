...
def visualize_top_k_results(
    ...
    if include_heatmaps:
        ...
        try:
            from visualization.similarity_heatmap import (
            ...
            print(
                f"      Warning: Heatmap generation failed ({e}), using standard visualization"
            )
...
def calculate_retrieval_metrics(
    ...
    # Recall@k
    recall = (
        len(retrieved_set & relevant_set) / len(relevant_set)
    ...
    return {"precision@k": precision, "recall@k": recall, "average_precision": ap}
...
def batch_process(
    items: List[Any], process_func, batch_size: int = 32, desc: str = "Processing"
) -> List[Any]:
    """
