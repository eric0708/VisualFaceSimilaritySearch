...
const SimilarityVisualizer = ({ 
  ...
  if (!queryPath || !resultPath) {
    return <Typography variant="body1">Select a sample and perform a search to start.</Typography>;
  }

  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, alignItems: 'center', width: '100%' }}>
      {/* Query Image Area */}
      <Paper elevation={3} sx={{ p: 1, position: 'relative', width: 'fit-content' }}>
         <Typography variant="subtitle2" align="center">Query Image</Typography>
             ...
             >
               {Array.from({ length: gridSize * gridSize }).map((_, idx) => (
                 <div
                   ...
                   style={{
                     border: hoveredQueryIdx === idx ? '1px solid white' : 'none',
                     backgroundColor: hoveredQueryIdx === idx ? 'rgba(255, 255, 255, 0.3)' : 'transparent'
                   }}
                 />
               ))}
             </Box>
      ...
      <Paper elevation={3} sx={{ p: 1, position: 'relative', width: 'fit-content' }}>
         <Typography variant="subtitle2" align="center">
            Result Image 
            {hoveredResultIdx !== null && heatmapData && heatmapData[hoveredResultIdx] !== undefined && (
              <span style={{ marginLeft: 10, fontWeight: 'bold' }}>
                Sim: {heatmapData[hoveredResultIdx].toFixed(3)}
              </span>
            )}
         </Typography>
              ...
              <>
                ...
                {/* Interactive Layer for Result Image */}
                 <Box 
                  ...
                  >
                    {heatmapData && heatmapData.map((val, idx) => (
                      <div
                        key={idx}
                        onMouseEnter={() => setHoveredResultIdx(idx)}
                        style={{
                          backgroundColor: colorScale ? colorScale(val) : `rgba(255,0,0,${val})`,
                          border: hoveredResultIdx === idx ? '1px solid white' : 'none',
                        }}
                      />
                    ))}
                  </Box>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, width: 300 }}>
        ...
        <div style={{ flex: 1, height: 10, background: 'linear-gradient(to right, purple, blue, green, yellow)' }} />
