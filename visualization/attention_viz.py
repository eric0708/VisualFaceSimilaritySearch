...
class AttentionVisualizer:
    ...
    def visualize_attention_map(self, attention_map: np.ndarray,
                               image_path: str,
    ...
    def visualize_multihead_attention(self, attention_weights: np.ndarray,
                                     image_path: str,
                                     save_dir: str,
                                     num_heads: int = 8):
        """
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(save_dir, 'multihead_attention.jpg'),
                   bbox_inches='tight', dpi=150)
        plt.close()
    
    def visualize_attention_flow(self, attention_map: np.ndarray,
    ...
    def compare_layer_attention(self, layer_attention_maps: Dict[int, np.ndarray],
                               image_path: str,
                               save_path: str):
        """
        ...
        for idx, (layer_idx, attn_map) in enumerate(sorted(layer_attention_maps.items()), 1):
            attn_vis = self.visualize_attention_map(
    ...
    def generate_attention_rollout(self, attention_weights_list: List[np.ndarray],
                                   image_path: str,
    ...
    def visualize_pairwise_attention(self, query_path: str,
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(save_dir, 'pairwise_attention.jpg'),
                   bbox_inches='tight', dpi=150)
        plt.close()
    ...
    try:
        ...
        if len(image_paths) >= 1:
            ...
            for layer_idx in layer_indices:
                attn_data_layer = embedder.extract_attention_maps(query_path, layer_idx=layer_idx)
                layer_attns[layer_idx] = attn_data_layer['attention_map']
            
            visualizer.compare_layer_attention(
                layer_attns, query_path,
